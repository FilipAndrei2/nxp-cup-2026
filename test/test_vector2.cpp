 ...
 * Tests for ls::Vector2<T> – construction, arithmetic, and geometric operations.
 *
 * NOTE: Vector2::Dot is implemented as std::sqrt(lhs.x*rhs.x + lhs.y*rhs.y)
 * which is NOT a standard dot product.  The tests document the actual
 * behaviour while noting the mathematical discrepancy.
 *
 * NOTE: Vector2::AngleBetween relies on the above Dot, so its formula is
 *   angle = acos( sqrt(a·b) / ... )
 * rather than the textbook  acos( a·b / (|a||b|) ).  The tests verify that
 * special cases (parallel / perpendicular) still produce the correct angles.
 */
...
TEST_CASE("Vector2::Cross – orthogonal unit vectors", "[vector2]") {
    // (1,0) × (0,1) = 1*1 - 0*0 = 1
    CHECK(FVector2::Cross(FVector2(1.0f, 0.0f), FVector2(0.0f, 1.0f)) == Approx(1.0f));
    // (0,1) × (1,0) = 0*0 - 1*1 = -1
    CHECK(FVector2::Cross(FVector2(0.0f, 1.0f), FVector2(1.0f, 0.0f)) == Approx(-1.0f));
}

TEST_CASE("Vector2::Cross – parallel vectors give zero", "[vector2]") {
    CHECK(FVector2::Cross(FVector2(1.0f, 0.0f), FVector2(2.0f, 0.0f)) == Approx(0.0f));
    CHECK(FVector2::Cross(FVector2(1.0f, 1.0f), FVector2(2.0f, 2.0f)) == Approx(0.0f));
}

// ── Dot product (as implemented – uses sqrt internally) ──────────────────────
// NOTE: The implementation computes sqrt(a.x*b.x + a.y*b.y), which is
// mathematically incorrect for a dot product but is what the code does.
// The tests below document the actual output of the implementation.

TEST_CASE("Vector2::Dot – parallel unit vectors (implementation behaviour)", "[vector2][dot]") {
    // sqrt(1*1 + 0*0) = 1  → same as true dot product for this case
    CHECK(FVector2::Dot(FVector2(1.0f, 0.0f), FVector2(1.0f, 0.0f)) == Approx(1.0f));
}

TEST_CASE("Vector2::Dot – orthogonal unit vectors (implementation behaviour)", "[vector2][dot]") {
    // sqrt(1*0 + 0*1) = 0  → same as true dot product for this case
    CHECK(FVector2::Dot(FVector2(1.0f, 0.0f), FVector2(0.0f, 1.0f)) == Approx(0.0f));
}

TEST_CASE("Vector2::Dot – anti-parallel vectors expose sqrt bug", "[vector2][dot]") {
    // True dot product of (1,0) · (-1,0) = -1
    // Implementation: sqrt(1*(-1) + 0*0) = sqrt(-1) → NaN (UB)
    // This demonstrates the mathematical incorrectness.
...
TEST_CASE("Vector2::AngleBetween – same direction (angle = 0)", "[vector2][angle]") {
...
TEST_CASE("Vector2::AngleBetween – perpendicular unit vectors (angle = PI/2)", "[vector2][angle]") {
