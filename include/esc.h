...
typedef struct {
  ...
  uint16 MaxDutyCycle;
  uint16 MedDutyCycle;
  int Speed;   /*values between -100 and 100: val >= 0 means Forward, val < 0
                  means Reverse*/
  uint8 Brake; /*value is zero or non zero: val == 0 means brake, val != 0 means
                  no brake*/
} Esc;
...
==================================================================================================*/
void EscInit(Pwm_ChannelType EscPwmChannel, uint16 MinDutyCycle,
             uint16 MedDutyCycle, uint16 MaxDutyCycle);
